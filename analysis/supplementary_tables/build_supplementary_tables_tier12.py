...
S7  Survival statistics (TCGA + lead gene KM)
S8  Spatial validation statistics (ROI vs non-ROI + MISTy + per-section)
S9  Treatment / clinical validation (DepMap, TCGA TvN, GSE cohorts)
S10 Pathway / hallmark enrichment
S11 TF activity, GeneSwitches, ligand-receptor, COMMOT
"""
...
with pd.ExcelWriter(OUT / "Supplementary_Table_S11_TF_LR_COMMOT.xlsx") as xw:
    ...
    if gs is not None:
        write_titled_sheet(xw, "GeneSwitches results",
            "Table S11c. GeneSwitches outcome  -  switch genes along diffusion pseudotime (sc.tl.dpt) (Fig 3 axis).",
            "switch_pseudotime_rank = order of switch event along pseudotime; mcfadden_R2 / nagelkerke_R2 = logistic-regression fit; direction = up/down.",
            gs)
